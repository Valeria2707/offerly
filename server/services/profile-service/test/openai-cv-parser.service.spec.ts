...
describe('OpenAiCvParserService', () => {
  it('fails predictably when AI parsing is not configured', async () => {
    const service = new OpenAiCvParserService(
      new ConfigService({ OPENAI_API_KEY: '', OPENAI_MODEL: '' })
    );

    await expect(
      service.parse('A sufficiently long CV text')
  ...
  it('normalizes links without a protocol before validating the structured result', async () => {
    ...
    const result = {
      basics: {
        ...
        location: null,
        summary: null,
        links: [{ label: 'LinkedIn', url: 'linkedin.com/in/jane' }]
      },
      preferences: {
