...
export class ProfileController {
  ...
  @ApiOperation({
    summary:
      'Get the authenticated user profile, creating an empty one when necessary'
  })
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      limits: { fileSize: CV_MAX_UPLOAD_SIZE_BYTES, files: 1 }
    })
  )
  @ApiConsumes('multipart/form-data')
  @ApiOperation({
    summary:
      'Temporarily process a PDF or DOCX CV and create a structured profile draft'
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiBadRequestResponse({
    description: 'File is missing or is not a valid PDF/DOCX'
  })
  @ApiPayloadTooLargeResponse({ description: 'File exceeds 10 MB' })
  @ApiUnprocessableEntityResponse({
    description: 'Document text or structured data could not be extracted'
  })
  @ApiServiceUnavailableResponse({
    description: 'AI parsing is not configured or the provider is unavailable'
  })
  importCv(
    @Req() request: AuthenticatedRequest,
  ...
  @Get('cv-imports/:id')
  @ApiOperation({
    summary: 'Get a CV import draft owned by the authenticated user'
  })
  @ApiOkResponse({ type: CvImportResponseDto })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Apply a reviewed CV draft to the authenticated user profile'
  })
  @ApiOkResponse({ type: ProfileResponseDto })
  @ApiBadRequestResponse({
    description: 'CV import is not ready to be applied'
  })
  @ApiNotFoundResponse({ description: 'CV import not found' })
  applyImport(
    @Req() request: AuthenticatedRequest,
